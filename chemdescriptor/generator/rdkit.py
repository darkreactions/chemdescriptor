...
class RDKitDescriptorGenerator(BaseDescriptorGenerator):
    ...
    def __init__(self, input_molecules, whitelist={}, command_dict={}, logfile=None):
        """Handles only SMILES and stores into self.molecules
        TODO: Handle InchI
        """
        super().__init__(logfile=logfile)

        self.descriptor_dict = {desc[0]: desc[1] for desc in Descriptors.descList}
        self.descriptor_list_3d = [
            desc for desc in dir(Descriptors3D) if "__" not in desc
        ]
        ...
        if whitelist:
            ...
            self.descriptor_whitelist = list(default_command_dict["descriptors"].keys())
        ...
        if command_dict:
            if isinstance(command_dict, dict):
                ...
                raise Exception(
                    "'command_dict' should be a dict. Found: {}".format(
                        type(command_dict)
                    )
                )
        else:
    ...
    def generate(self, output_file_path, dataframe=False):
        ...
        for smi in self.smiles:
            ...
            if molecule:
                ...
                self.logger.error("{} did not generate a mol object".format(smi))
